...
<?php
	...
	$tamdiv2 = Selecionar_Campos_a($servername,$username,$password,$dbname,$table,testando,tam_div) * $tampixl;
...
?>
	...
	<div style="position: fixed; left: 100px; width: <?php if($WID > 800){	$WID = 800;	echo $WID; }else{	echo $WID + 5;}
...
."px; height: ".$PASS_HEIGHT ."px; background-color: #6c7ae0; text-align: $TEXT_ALIGN; font-size: 15px; color: white;\">";
		...
		while($PASS_REGS < $TOTAL_REGS){
			while($PASS_COL < $DIVIDE_COL){
				echo "<div style=\"position: absolute; top: ".$PASS_TOP ."px; left: " .$PASS_LEFT ."px; width: ".($TAM_PERCOL[$PASS_COL][1] * $tampixl)
."px; height: ".$PASS_HEIGHT ."px; background-color: " .$DIVCOLOR[$PASS_REGS % 2] ."; text-align: $TEXT_ALIGN; font-size: 15px;\">";
